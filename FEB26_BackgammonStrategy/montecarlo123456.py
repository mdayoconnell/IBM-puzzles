...
from backgammon_strategy123456 import (
    Action,
    Outcome,
    State,
    is_blot,
    make_dp_solver,
    make_turn_action_cache,
    validate_outcomes,
)
...
def make_default_outcomes(n_men: int) -> List[Outcome]:
    k_max_double = min(4, n_men)
    k_max_non_double = min(2, n_men)
    outcomes: List[Outcome] = []
    for i in range(1, 7):
        for j in range(i, 7):
            if i == j:
                outcomes.append(
                    Outcome(f"{i}{j}", 1 / 36, (i, i, i, i), 1, k_max_double)
                )
            else:
                outcomes.append(Outcome(f"{i}{j}", 2 / 36, (i, j), 1, k_max_non_double))
    return outcomes
...
def make_optimal_policy(
    ...
    V, _best_action, _best_transition, _s0, _used_outcomes, _timing = make_dp_solver(
        n_men=n_men,
        p_max=p_max,
...
def estimate_ev(
    ...
    turn_actions_cached = make_turn_action_cache(list(outcomes))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Monte Carlo solver for the d6 game.")
